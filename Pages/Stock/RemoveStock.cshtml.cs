...
namespace InventorySystem.Pages.Stock
{
    public class RemoveStockModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            {
                await LoadProductsAsync();
                ErrorMessage = $"Cannot remove {Quantity} units. Current stock is only {product.QuantityInStock} units.";
                return Page();
            }
            ...
            {
                await LoadProductsAsync();
                ErrorMessage = "Please select a reason for removing stock.";
                return Page();
            }

            try
            {
                var success = await _stockService.RemoveStockAsync(ProductId, Quantity, Notes);
                ...
                {
                    await LoadProductsAsync();
                    ErrorMessage = "Failed to remove stock. Please try again.";
                    return Page();
                }
            }
            catch (Exception ex)
        ...
        private async Task LoadProductsAsync()
        {
            ...
            var productList = products
                ...
                {
                    Value = p.Id.ToString(),
                    Text = $"{p.Name} (SKU: {p.SKU}) - Stock: {p.QuantityInStock}",
                    Disabled = p.QuantityInStock == 0
                })
                .OrderByDescending(p => p.Disabled)
                .ToList();
